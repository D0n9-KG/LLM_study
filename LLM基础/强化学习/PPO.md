...
因无法遍历所有轨迹，所以需要用蒙特卡洛采样来近似期望，即从策略 $P_{\theta}(\tau)$ 中采样N条轨迹，用“样本均值”替代理论期望。
...
2. $\sum_{t=1}^{T_n} R(\tau^n) \nabla \log P_{\theta}(a_n^t | s_n^t)$：利用累积回报$R(\tau^n)$ 加权每一步的对数梯度，体现策略梯度方法中的优势估计。
3. $\sum_{t=1}^{T_n} R(\tau^n) \log P_{\theta}(a_n^t | s_n^t)$：省略梯度符号后的形式，通常用于描述带有加权对数概率的情况。
...
但这明显是存在改进空间的，因为我们只是简单地使用回报来调整策略，而没有考虑到**回报的分布**，这样就会导致回报的方差较大，训练不稳定。
...
还有一种情况会影响我们**算法的稳定性**，那就是在好的局势下和坏的局势下。比如在好的局势下，不论你做什么动作，你都会得到正的回报，这样算法就会增加所有动作的概率。
得到reward大的动作的概率大一些，但是这样会让训练很慢，也会不稳定。最好是能够让相对好的动作的概率增加，相对坏的动作的概率减小。

为了解决这个问题，我们可以**对所有动作的reward都减去一个baseline**，这样就可以让相对好的动作的reward增加，相对坏的动作的reward减小，也能反映这个动作**相对其他动作的价值**。
...
- State-Value Function：$V_{\theta}(s)$ 表示在状态 $s$ 下的价值，即从状态 $s$ 开始，按照某个策略 $\pi$ 执行，最终**获得的回报的期望值**。$V(s)$ 可以用来评估在状态 $s$ 下的好坏，从而指导智能体的决策，即状态价值函数。
...
$Q_{\theta}(s, a)$ 表示在状态 $s$ 下采取动作 $a$ 的价值，$V_{\theta}(s)$ 表示在状态 $s$ 下的价值。我们来看一下下面这个公式（把$Q_\theta(s_t, a)$ 分为即时奖励和未来状态价值两部分）：
...
我们可以看到，现在优势函数中只剩下了状态价值函数 $V_\theta(s_t)$ 和下一个状态的价值函数 $V_\theta(s_{t+1})$，这样就由原来需要**训练两个神经网络变成了只需要训练一个状态价值网络**，这样就减少了训练的复杂度。
...
我们知道，采样的步数越多，会导致方差越大，但偏差会越小。（**步数少会对长期奖励低估**）
...
2. $\delta_{t+1}^V$：是时间步 $t+1$ 的优势函数，类似地表示在时刻 $ t+1 $ 获得的即时奖励 $ r_{t+1}$ 加上状态 $ s_{t+2} $ 的折扣价值 $ \gamma \cdot V_\theta(s_{t+2}) $减去状态 $ s_{t+1} $ 的价值估计 $ V_\theta(s_{t+1}) $。
   ...
   ![image-20251201211311073](C:\Users\jd\AppData\Roaming\Typora\typora-user-images\image-20251201211311073.png)

   那我们究竟要采样几步呢？介绍一下广义优势估计GAE（Generalized Advantage Estimation）——**全都用**
...
原始的策略梯度需要在每次更新策略后重新采样轨迹，效率极低。

PPO 是 OpenAI 提出的一种基于策略梯度的强化学习算法，它通过对策略梯度的优化，来提高策略的稳定性和收敛速度。PPO 算法的核心思想是在更新策略时，通过引入一个**重要性采样比例**，来限制策略更新的幅度，从而保证策略的稳定性。

![image-20251203113021088](C:\Users\jd\AppData\Roaming\Typora\typora-user-images\image-20251203113021088.png)
...
PPO截断（PPO-Clipped）是 PPO 的另一种变体，它通过对比新旧策略的比值，来**限制策略更新的幅度**，从而保证策略的稳定性。具体来说，PPO-Clipped 的目标函数为：
